...
var index_schema =	function (rows) {
						return	rows .filter (function (row) {
									return row .category && row .spreadsheet
								}) .map (function (row) {
									return	{
...
var category_schema =	function (rows) {
							return	rows .filter (function (row) {
...
var kk_index =	function (id) {
					...
					return	promisify (index .useServiceAccountAuth) (kk_account)
								...
								.then (function (info) {
									...
									return index_schema (rows);
								}) .then (function (categories) {
									return	categories .map (function (category) { 
												return	kk_category (category .id) .then (function (kk_category) {
															return	{
																		name: category .category,
																		category:	kk_category .map (function (question) {
																						question .id = category .id + '__' + question .id;
																						return question;
																					})
																	};
														})
									...
									return	indexes .reduce (function (total, category) {
												total [category .name] = (total [category .name] || []) .concat (category .category)
												return total;
											}, {})
								})
				};
var kk_category =	function (id) {
